...
void printTaintedVars(map<string, pathType> analysisMap) {
	errs() << "TAINTED VARIABLES:\n";
    for (auto& row : analysisMap){
    	pathType taintedPath = row.second;
    	string BBLabel = row.first;
...
// USAGE taint_analysis trace.ll p/t
// p: prints taint vars and paths (DEFAULT)
// t: prints only taint vars
int main(int argc, char **argv) {
	...
	if(MODE == 'p')
		...
		printTaintedVars(taintAnalysisMap);
